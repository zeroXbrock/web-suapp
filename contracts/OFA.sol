...
contract OFAPrivate {
    ...
    function newMatch(
        Suave.DataId shareDataRecordId,
        uint64 decryptionCondition
    ) external payable returns (bytes memory) {
        HintOrder memory hintOrder = saveOrder(decryptionCondition);
        ...
        Suave.confidentialStore(
            ...
            abi.encode(dataRecords)
        );

        return abi.encodeWithSelector(this.emitHint.selector, hintOrder);
    }

    function emitMatchDataRecordAndHintCallback(
        string memory bundleRawResponse
    ) external payable {
        emit BundleEmitted(bundleRawResponse);
    }

    function emitMatchDataRecordAndHint(
        string memory builderUrl,
        Suave.DataId dataRecordId
    ) external payable returns (bytes memory) {
        ...
        return
            abi.encodeWithSelector(
                this.emitMatchDataRecordAndHintCallback.selector,
                response
            );
    }
