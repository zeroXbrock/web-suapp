...
import { suaveRigil, holesky } from '@flashbots/suave-viem/chains'
...
// public testnet node, may need to change if it goes down:
const TESTNET_RPC_URL_HTTP: string = 'https://holesky.rigil.suave.flashbots.net'

const testnetWallet = createWalletClient({
  account: privateKeyToAccount(
    '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80',
  ),
  chain: holesky,
  transport: http(TESTNET_RPC_URL_HTTP),
})
const testnetProvider = createPublicClient({
  transport: http(TESTNET_RPC_URL_HTTP),
  chain: holesky,
})
...
export function setupSendBidButton(
  ...
  const sendBid = async (suaveWallet: any) => {
    ...
    const signedTx = await testnetWallet.signTransaction(sampleTx)
    console.log('signed testnet tx', signedTx)

    // create bid & send ccr
    const decryptionCondition = 1n + (await testnetProvider.getBlockNumber())
